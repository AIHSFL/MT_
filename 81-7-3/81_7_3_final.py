...
Original file is located at
    https://colab.research.google.com/drive/1f7SC_ifl86dlxf5l41VIRcus0KPmitCf
"""

'''
->vgl. 81-7-2: mehr hidden layer+ nicht pyramid
->vgl 81-7-1: relu, lernrate noch kleiner, noch mehr daten, modell teiefer, schmaller
...
from tensorflow.keras.layers import Input, Dense, Concatenate,LeakyReLU, LayerNormalization, Dropout
...
x = Dense(512, activation='relu', kernel_initializer='he_normal', kernel_regularizer=l2(1e-5))(input_layer)
x = Dropout(0.2)(x)
x = Dense(1024, activation='relu', kernel_initializer='he_normal', kernel_regularizer=l2(1e-5))(x)
x = Dropout(0.2)(x)
x = Dense(512, activation='relu', kernel_initializer='he_normal', kernel_regularizer=l2(1e-5))(x)
x = Dropout(0.2)(x)
x = Dense(256, activation='relu', kernel_initializer='he_normal', kernel_regularizer=l2(1e-5))(x)
x = LayerNormalization()(x)

p = Dense(128, activation='relu', kernel_initializer='he_normal')(x); p = Dropout(0.2)(p)
k = Dense(128, activation='relu', kernel_initializer='he_normal')(x); k = Dropout(0.2)(k)

p_out = Dense(BIT_LENGTH, activation='sigmoid', name='p_out')(p)
k_out = Dense(BIT_LENGTH, activation='sigmoid', name='k_out')(k)
